...
void initial_ports() {
	...
	BUTTs_ddr &= ~(1<<butt1) & ~(1<<butt2);		//initialize port C on the button
	BUTTs |= (1<<butt1) | (1<<butt2);		    //pull up buttons to 5V
}
/*----------------------------------------------------------------------*/
...
int main(void) {
	...
	while (1) {
		if (BUTTS_READ & (1 << butt2)) {   //?push the button for a logical zero
